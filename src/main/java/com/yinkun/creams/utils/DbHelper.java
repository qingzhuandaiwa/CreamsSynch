		...
		synchronized (CONFIG_NAME) {
			...
			if (!status) {
				...
				if (StrKit.isBlank(imDbConfig.getType()))
					imDbConfig.setType(new Config().getDbType());

				if (!imDbConfig.getShowSql())
					imDbConfig.setShowSql(new Config().getDbShowSql());

				String url = "";
				String username = "";
				String password = "";
				url = new Config().getDbUrl();
				username = new Config().getDbUserame();
				password = new Config().getDbPassword();
